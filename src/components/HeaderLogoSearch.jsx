...
const HeaderLogoSearch = () => {
  ...
  //----- 검색창 광고 슬라이드
  useEffect(() => {
    const interval = setInterval(() => {                                // setInterval(실행할 함수, 시간) 일정시간마다 함수 실행 >> 10초 설정 >> setInterval은 JS 내장 함수
      setIndex((prev) => (prev + 1) % searchAdText.length);             // setIndex : 이전 값에 +1을 하도록 하고 length와 같아지면 초기화 되는
    }, 5000);

    return () => clearInterval(interval);                               // return : 컴포넌트가 화면에서 사라지면 cleanUp 해주는 것
  }, [index]);                                                          // [] : 컴포넌트 시작할 때 한번만 >> [index] : 버튼 누르면 초기화되도록
  ...
  // 외부 클릭시 open(false)
  useEffect(() => {
    const clickOutside = (e) => {                                                       // clickOutside : 클릭 이벤트시 실행되는 함수 >> e : 이벤트 객체
      if (wrapperRef.current && wrapperRef.current.contains(e.target)) {                // wrapperRef.current : 내가 지정한 wrapper의 영역
        return;                                                                         // wrapperRef.current.contains(e.target) : 내가 클릭한 곳이 영역에 포함되었는가
      }
      setOpen(false);
    }
    document.addEventListener("mousedown", clickOutside);                               // document :  웹 페이지 전체 >> addEventListener : 이벤트 등록 함수
                                                                                        // mousedown : 이벤트 종류 (마우스 누르는 순간) >> mouseup(마우스 뗄 때), click(눌렀다 뗀 후)
    return () => {                                                                      // return : 컴포넌트가 화면에서 사라지면 cleanUp 해주는 것
